...
class VIMeasure(Metric):
  '''
  This calculates variation of information.
  '''
  ...
  def apply(cls, label_array):
    ...
    for z in range(label_array.shape[0]-1):
      vi_sum += partition_comparison.variation_of_information(label_array[z,:,:].astype(np.uint64).ravel(),
