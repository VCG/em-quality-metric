...
def build_cnn2(input_var=None):
    ...
    # Input layer, as usual:
    network2 = lasagne.layers.InputLayer(shape=(None, 1, 28, 28),
                                        input_var=input_var)
    # This time we do not apply input dropout, as it tends to work less well
    # for convolutional layers.
    ...
    network2 = lasagne.layers.Conv2DLayer(
            network2, num_filters=32, filter_size=(5, 5),
            nonlinearity=lasagne.nonlinearities.rectify,
            W=lasagne.init.GlorotUniform())
    # Expert note: Lasagne provides alternative convolutional layers that
    # override Theano's choice of which implementation to use; for details
    ...
    # merge layer
    merged = lasagne.layers.ConcatLayer([network, network2])
    ...
    # A fully-connected layer of 256 units with 50% dropout on its inputs:
    merged = lasagne.layers.DenseLayer(
