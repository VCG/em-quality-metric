...
class TrainCNN(object):
  ...
  def run(self, num_epochs=5):
    ...
    layers = self.build(image_var, prob_var, binary_var, overlap_var, self._THIRD_CONV_LAYER)
    ...
    train_fn = theano.function([image_var, prob_var, binary_var, overlap_var, target_var], loss, updates=updates)

    # Compile a second function computing the validation loss and accuracy:
    val_fn = theano.function([image_var, prob_var, binary_var, overlap_var, target_var], [test_loss, test_acc])
    ...
    for epoch in range(self._EPOCHS):
        ...
        for batch in self.iterate_minibatches(X_train, y_train, self._BATCH_SIZE, shuffle=False):
            ...
            train_err += train_fn(images, probs, binarys, overlaps, targets)
            train_batches += 1
  ...
  def build(self, input_image, input_prob, input_binary, input_overlap, third_conv_layer=False):
    '''
    '''
    ...
    merged = lasagne.layers.ConcatLayer([layers['image']['network'], layers['prob']['network'], layers['binary']['network'], layers['overlap']['network']])
