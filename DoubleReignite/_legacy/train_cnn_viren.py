...
class TrainCNN(object):
  ...
  def run(self, num_epochs=5):
    ...
    binary_var = T.tensor4('binary')
    # binary2_var = T.tensor4('binary2')
    #overlap_var = T.tensor4('overlap')
    dyn_obj_var = T.tensor4('dyn_obj')
    dyn_bnd_var = T.tensor4('dyn_bnd')
    target_var = T.ivector('targets')

    # layers = self.build(image_var, prob_var, binary1_var, binary2_var, overlap_var, self._THIRD_CONV_LAYER)
    layers = self.build(image_var, prob_var, binary_var, dyn_obj_var, dyn_bnd_var, self._THIRD_CONV_LAYER)
    ...
    train_fn = theano.function([image_var, prob_var, binary_var, dyn_obj_var, dyn_bnd_var, target_var], loss, updates=updates)

    # Compile a second function computing the validation loss and accuracy:
    val_fn = theano.function([image_var, prob_var, binary_var, dyn_obj_var, dyn_bnd_var, target_var], [test_loss, test_acc])
    ...
    for epoch in range(self._EPOCHS):
        ...
        for batch in self.iterate_minibatches(X_train, y_train, self._BATCH_SIZE, shuffle=False):
            images, probs, binarys, dyn_objs, dyn_bnds, targets = batch
            # self._dbg = images, probs, binarys
            train_err += train_fn(images, probs, binarys, dyn_objs, dyn_bnds, targets)
            train_batches += 1
        ...
        for batch in self.iterate_minibatches(X_val, y_val, self._BATCH_SIZE, shuffle=False):
            ...
            images, probs, binarys, dyn_objs, dyn_bnds, targets = batch
            err, acc = val_fn(images, probs, binarys, dyn_objs, dyn_bnds, targets)
            val_err += err
            val_acc += acc
            val_batches += 1
    ...
    binary_var = layers['binary']['input_layer'].input_var
    dyn_obj_var = layers['dyn_obj']['input_layer'].input_var
    dyn_bnd_var = layers['dyn_bnd']['input_layer'].input_var
  ...
  def build(self, input_image, input_prob, input_binary, input_dyn_obj, input_dyn_bnd, third_conv_layer=False):
    '''
    '''
    ...
    layers = self.gen_network(layers, 'binary', input_binary, third_conv_layer)
    layers = self.gen_network(layers, 'dyn_obj', input_dyn_obj, third_conv_layer)
    layers = self.gen_network(layers, 'dyn_bnd', input_dyn_bnd, third_conv_layer)
    ...
    merged = lasagne.layers.ConcatLayer([layers['image']['network'],
                                         layers['prob']['network'],
                                         layers['binary']['network'],
                                         layers['dyn_obj']['network'],
                                         layers['dyn_bnd']['network']])
  ...
  def iterate_minibatches(self, inputs, targets, batchsize, shuffle=False):
      ...
      for start_idx in range(0, len(inputs['image']) - batchsize + 1, batchsize):
          ...
          yield inputs['image'][excerpt], inputs['prob'][excerpt], inputs['merged_array'][excerpt], inputs['dyn_obj'][excerpt], inputs['dyn_bnd'][excerpt], targets[excerpt]
