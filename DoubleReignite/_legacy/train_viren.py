...
if __name__ == '__main__':
  ...
  def store_filters(s, layers, epoch):
    ...
    binary_filters = Image.fromarray(s.visualize_filters(layers['binary']['c2d_layer']))
    binary_filters.save(OUTPUT_PATH+os.sep+'binary_filters_'+str(epoch)+'.png')
    dyn_obj_filters = Image.fromarray(s.visualize_filters(layers['dyn_obj']['c2d_layer']))
    dyn_obj_filters.save(OUTPUT_PATH+os.sep+'dyn_obj_'+str(epoch)+'.png')
    dyn_bnd_filters = Image.fromarray(s.visualize_filters(layers['dyn_bnd']['c2d_layer']))
    dyn_bnd_filters.save(OUTPUT_PATH+os.sep+'dyn_bnd_'+str(epoch)+'.png')
