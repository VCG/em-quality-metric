...
class OurConv2DLayer(lasagne.layers.Conv2DLayer):

    def __init__(self, incoming, num_filters, filter_size, stride=(1, 1),
                 pad=0, untie_biases=False,
                 W=init.GlorotUniform(), b=init.Constant(0.),
                 nonlinearity=nonlinearities.rectify,
                 convolution=T.nnet.conv2d, binary_mask=None, **kwargs):
        super(OurConv2DLayer, self).__init__(incoming, num_filters, filter_size, **kwargs)
    ...
    def get_output_for(self, input, input_shape=None, **kwargs):
        ...
        output_dyn_pool = output_before = self.nonlinearity(activation)

        if self._binary_mask:
          b_m_binary_mask = self._binary_mask.shape[0]

          half_filter_size = int(np.floor(self.filter_size[0]/2))

          out = theano.tensor.extra_ops.repeat(self._binary_mask/self._binary_mask.max(), self.num_filters, 1)


          # output_dyn_pool = output_before
          # out = np.zeros((self._binary_mask.shape[0], int(self.num_filters), self._binary_mask.shape[2], self._binary_mask.shape[3]))
          # for i in range(0,self.num_filters):
          #   out[:, i, :, :] = self._binary_mask



          output_dyn_pool = output_dyn_pool * out[:,:, half_filter_size:-half_filter_size, half_filter_size:-half_filter_size]

        return output_dyn_pool
...
class TrainCNN(object):
  ...
  def run(self, num_epochs=5):
    ...
    binary_mask_var = T.tensor4('binary_mask')

    theano_function_vars = theano_vars + [binary_mask_var] + [target_var]

    

    # layers = self.build(image_var, prob_var, binary1_var, binary2_var, overlap_var, self._THIRD_CONV_LAYER)
    layers = self.build(theano_vars, binary_mask_var, self._THIRD_CONV_LAYER)
    ...
    # # hacking_input 
    # hacking_size_fn = theano.function([theano_vars[0]], [lasagne.layers.get_output(layers['image']['c2d_layer'], deterministic=True)])
    

    # X_train_rotated = X_train

    # test_batch = self.iterate_minibatches(X_train_rotated, y_train, 1, shuffle=False)
    # print hacking_size_fn(list(test_batch)[0][0])[0].shape
  ...
  def gen_network(self, layers, name, input_var, binary_mask_var, third_conv_layer=False):
    '''
    '''
    ...
    c2d_layer = OurConv2DLayer(      
            input_layer, num_filters=self._NO_FILTERS, filter_size=self._FILTER_SIZE,
            nonlinearity=lasagne.nonlinearities.rectify,
            W=lasagne.init.GlorotUniform(), binary_mask=binary_mask_var)
    ...
    # max_pool_layer = lasagne.layers.Pool2DLayer(c2d_layer, pool_size=(2, 2), mode='average_inc_pad')
  ...
  def build(self, input_vars, binary_mask_var, third_conv_layer=False):
    '''
    '''
    ...
    for e,i in enumerate(self._inputs):
      layers = self.gen_network(layers, i, input_vars[e], binary_mask_var, third_conv_layer)
  ...
  def iterate_minibatches(self, inputs, targets, batchsize, shuffle=False):
      ...
      for start_idx in range(0, len(inputs['image']) - batchsize + 1, batchsize):
          ...
          minibatches.append(inputs['dyn_bnd_dyn_obj'][excerpt])            
          minibatches.append(targets[excerpt])
          
          # minibatches.append(inputs['dyn_obj'][excerpt])
